...
pub fn save_review_note(note: &ReviewNote) -> Result<(), String> {
    ...
    conn.execute(
        ...
        INSERT INTO review_notes (
            id, target_key, scope, file_path, line, side, start_line, start_side, reply_to_id, body, author, created_at
        )
...
pub fn delete_all_review_notes(target_key: &str, scope: &str) -> Result<usize, String> {
    let conn = super::open_cache_connection()?;
    delete_all_review_notes_with_connection(&conn, target_key, scope)
}
...
fn read_review_notes_with_connection(
    ...
    let mut statement = conn
        .prepare(
            "
            SELECT id, target_key, scope, file_path, line, side, start_line, start_side, reply_to_id, body, author, created_at
            FROM review_notes
            WHERE target_key = ?1
              AND scope = ?2
...
fn delete_all_review_notes_with_connection(
    ...
    conn.execute(
        "DELETE FROM review_notes WHERE target_key = ?1 AND scope = ?2",
        params![target_key, scope],
    )
