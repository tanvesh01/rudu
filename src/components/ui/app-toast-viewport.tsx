...
function getToastRootClassName(toast: ToastObject<AppToastData>) {
  ...
  const placementClassName =
    placement === "bottom-center" || placement === "center"
      ? "self-center"
      : "self-end";
...
export function AppToastViewport() {
  ...
  const centerToasts = toasts.filter(
    (toast) => toast.data?.placement === "center",
  );
  const bottomToasts = toasts.filter(
    (toast) => toast.data?.placement !== "center",
  );

  function renderToast(toast: ToastObject<AppToastData>) {
    return (
      <Toast.Root
  ...
  return (
    <Toast.Portal>
      ...
      <Toast.Viewport className="pointer-events-none fixed inset-4 z-50 flex items-center justify-center">
        <div className="flex flex-col gap-2">{centerToasts.map(renderToast)}</div>
      </Toast.Viewport>
    </Toast.Portal>
  );
}
