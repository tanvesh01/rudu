...
type RepoOpenActions = {
  repoAccordionToggled: (repo: string, open: boolean) => void;
  syncRepos: (repoNames: string[]) => void;
};

type RepoOpenStore = RepoOpenState & {
  actions: RepoOpenActions;
};

const useRepoOpenStore = create<RepoOpenStore>((set) => ({
  openRepoValues: [],

  actions: {
    repoAccordionToggled(repo, open) {
      set((state) => {
        if (open) {
        ...
        return {
          openRepoValues: state.openRepoValues.filter(
            (value) => value !== repo,
          ),
        };
      });
    ...
    syncRepos(repoNames) {
      set((state) => {
        ...
        if (
          ...
          nextOpenRepos.every(
            (repoName, index) => repoName === state.openRepoValues[index],
          )
        ) {
          return state;
        }
